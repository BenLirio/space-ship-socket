...
export function spawnProjectile(loop: InternalLoopState, ship: ShipState) {
  ...
  const origins = (
    ship.bulletOrigins && ship.bulletOrigins.length
      ? ship.bulletOrigins
      : [
          { x: -10, y: -30 },
          { x: 10, y: -30 },
  ...
  const now = Date.now();
  origins.forEach(({ x: localX, y: localY }) => {
    ...
    loop.gameState.projectiles.push({
...
export function simulateProjectiles(loop: InternalLoopState, dt: number) {
  ...
  loop.gameState.projectiles.forEach((p) => {
    const age = now - p.createdAt;
    if (age > PROJECTILE_LIFETIME_MS) return; // despawn
    ...
    for (const [shipId, ship] of Object.entries(loop.gameState.ships)) {
      if (shipId === p.ownerId) continue;
      if ((ship.health ?? 0) <= 0) continue;
      ...
      if (!(cx < minX || cx > maxX || cy < minY || cy > maxY)) {
        ...
        if (a > 1e-12) {
          ...
          if (disc >= 0) {
            ...
            if ((t1 >= 0 && t1 <= 1) || (t2 >= 0 && t2 <= 1)) intersects = true;
          }
        } else if (c <= 0) {
          intersects = true;
        }
